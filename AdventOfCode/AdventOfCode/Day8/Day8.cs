...
public class Day8
{
    public long Part1(string[] input)
    {
        ...
        var nodes = input[2..].Select(x =>
        {
            var split = x.Split("=");
            var neighbors = split[1]
                .Replace("(", "")
                .Replace(")", "")
                .Split(",");

            return new Node(split[0].Trim(), neighbors[0].Trim(), neighbors[1].Trim());
        }).ToList();

        var currentNode = nodes.First(x => x.NodeName == "AAA");
        var i = command.Length;
        var counter = 0;
        while (currentNode.NodeName != "ZZZ")
        {
            var action = command[counter % i];
            currentNode = action == 'R' ? 
                nodes.First(x => x.NodeName == currentNode.RightNode) : 
                nodes.First(x => x.NodeName == currentNode.LeftNode);
    ...
    private record Node(string NodeName, string LeftNode, string RightNode);
